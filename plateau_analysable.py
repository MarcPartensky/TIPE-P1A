...
class PlateauAnalysable(Plateau):
    ...
    def presenterPionsStables(self,fenetre): #Obselète
        ...
        for i in range(2):
            ...
            self.presenter(pions,self.pieces_couleur[i],fenetre,message="pions stables",pause=True)
    ...
    def estUnPionStable(self,pion,fenetre,niveau=1): #Cette fonction a été remplacée par le concept de stabilité d'Alexandre.
        ...
        cote_oppose=self.obtenirCoteOppose(cote) #Récupère le côté opposé avec le côté du joueur.
        mouvements=self.obtenirMouvementsValides(cote_oppose) #Récupère tous les mouvements enemis.
        ...
        if not self.estUnPionPrenable(pion): #Si c'est le cas, il est stable.
            ...
            for mouvement in mouvements: #Fait une itération de chaque mouvement un par un.
                nouveau_plateau=copy.deepcopy(self) #On copie le plateau pour simuler chaque mouvement ennemi.
                nouveau_plateau.placerPion(mouvement,cote_oppose) #On joue le mouvement ennemi itéré dans le nouveau plateau.
                nouveau_cote=nouveau_plateau.obtenirCase(pion) #On récupère la couleur du pion après le mouvement enemi.
                if nouveau_cote!=cote: #Détermine si ce pion est pris i.e. si la couleur de ce pion dans le nouveau plateau est différente de celle d'avant.
    ...
    def test_si_le_joueur_cote_peut_prendre_position(self, cote, positions): # c'est utilisé ? 
        ...
        if isinstance(positions, tuple) :
            if len(positions)==2: #'positions' n'est pas une liste de positions mais juste une couple de coordonnées.
                postions=[positions]
    ...
    def Nombre_pion_retourne(self, pos, cote):
        ...
        return nombre_final-nombre_init-1 #-1 Car on pose un pion et ce pion n'a pas été retourné.
