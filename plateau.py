...
class Plateau:
    ...
    def obtenirLignesAlentours(self, position):
        ...
        directions = self.obtenirDirections()
        lignes = [
            self.obtenirLigneExclus(position, direction) for direction in directions
        ]
        return lignes
    ...
    def estMouvementValideDansLigne(self, cote, ligne):
        ...
        for position in ligne:  # Regarde chaque pion de la ligne dans l'ordre.
            if not self.estDansGrille(
                position
            ):  # Si on sort de la ligne, celle-ci ne peut pas être prise
                ...
                if case == cote:  # Si la case est a nous
                    if (
                        possible
                    ):  # et que l'on a déja rencontré des pions ennemis on peut prendre cette ligne.
                        valide = True
                    break  # On arrete la vérification
                elif (
                    case == cote_oppose
                ):  # Si la case contient un pion ennemi alors on peut possiblement le prendre
                    possible = True
    ...
    def presenter(
        ...
        if self.demonstration:
            ...
            if positions:
                ...
                if message:
                    position = self.obtenirPositionBrute(
                        [p - 2 / 5 for p in positions[0]]
                    )
    ...
    def colorerCase(self, positions, couleur):
        ...
        for position in positions:
            ...
            )  # Taille d'un panneau en coordonnees de ce panneau
            ...
            for i in range(2, 6):
                pygame.draw.rect(
                    self.surface, couleur, [mx + i, my + i, cx - 2 * i, cy - 2 * i], 1
                )
    ...
    def afficher(self):
        ...
        if cfg.THEME_PLATEAU[
            "aide mouvement"
        ]:  # cela correspond à l'affichage des rond rouge qui montre les coup possible par le joueur
    ...
    def afficherGrille(self):
        ...
        for y in range(sy):
            ...
            pygame.draw.line(
                self.surface, cfg.THEME_PLATEAU["couleur grille"], start, end, 1
            )
        for x in range(sx):
            _x = x * wsx // sx
            start = (_x, 0)
    ...
    def afficherDecorationGrille(self):
        ...
        for position in positions_points_graphic:
            pygame.draw.circle(
                self.surface, cfg.THEME_PLATEAU["couleur grille"], position, rayon, 0
            )
    ...
    def afficherPions(self):
        ...
        sx, sy = self.taille
        rayon = int(
            min(wsx, wsy) / min(sx, sy) * cfg.THEME_PLATEAU["rayon relatif pion"]
        )
        for y in range(sy):
            for x in range(sx):
                case = self.obtenirCase((x, y))
    ...
    def afficherAnimation(self, choix):
        ...
        pygame.draw.circle(
            self.surface,
            cfg.THEME_PLATEAU["couleur animation pion"],
            position,
            rayon + 2,
            0,
        )
